...
• *Cooked before?* (checkbox) — for 📖 Recipe items only: check if you've made it at least once
...
• 📖 Recipe — recipe reference cards, not to-dos. These are never "done" — they're permanent references. Use the *Cooked before?* checkbox to track whether you've made it at least once. Don't assign Urgency or Status to recipes.
...
*Page appearance:*
• Set an icon emoji on every page — parent tasks AND subtasks — that matches the task type or content (e.g. 🍝 for a recipe, 🛒 for shopping, 🏠 for home tasks, ✉️ for a letter, 👷 for tradespeople)
• Subtasks should have their own distinct icon that reflects their specific action (e.g. parent 🏦, subtask 1 ✉️, subtask 2 🔒)
• Set a random cover image from Notion's built-in gradient/color collection (use the "color" type, not external URLs)

*WhatsApp confirmations:*
• When confirming a created task in chat, lead the task name with the same emoji used as the Notion page icon
• Subtasks listed in chat should also show their individual emoji
